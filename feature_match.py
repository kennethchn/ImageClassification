...
#    res = svm_clf.predict( [[5.5, 1.7]] )
    ...
    while key_list:
        ...
        if not os.path.exists(cfd_folder_name):
            ...
            print('ErrorMessage:', cfd_folder_name, 'is already exisit!')
            return -1
...
def demo():
    ...
    img_name, img_kp, img_des = rw_feature.read_feature(feature_path_dict[key_list[0]].strip())
    print( img_name )
    img_name2, img_kp2, img_des2 = rw_feature.read_feature(feature_path_dict[key_list[100]].strip())
    print( img_name2)
